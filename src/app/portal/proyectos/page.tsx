...
export default async function PortalProyectos() {
  ...
  const [{ data: projects }, { data: quotes }] = await Promise.all([
    ...
    supabase.from("project_quotes").select("project_id, amount, is_winner").eq("organization_id", res.orgId),
  ]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted">
          Las cotizaciones y la empresa elegida para cada proyecto del PH — todo a la vista.
        </p>
      </div>
