...
- **Fase 6c — pendiente (hallazgos diferidos de la auditoría):** **XIII mes** dedicado (aplicar `contribution_rates.css_xiii` 7.25%, sin seg. educativo — sembrado pero no usado aún); **planilla por período** (batch usando payroll_periods/items; hoy es preview, y `payroll_items` tiene columnas Panamá-específicas → usar `detail jsonb` al persistir); **PDFs** (recibo, carta de liquidación); **prima de antigüedad y promedio 6m desde `salary_history`/ledger** (hoy usa base × meses como proxy; exacto solo si no hubo aumentos); `xiiiPeriodStart` hardcodea cuatrimestres PA → mover a datos al sumar 2º país; campo `country_code` en el form de empleado; atomicidad de createEmployee/saveLiquidation vía RPC.
